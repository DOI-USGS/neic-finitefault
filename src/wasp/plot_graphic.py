...
def plot_misfit(used_data_type, forward=False, event=None):
    ...
    if "dart" in used_data_type:
        ...
        traces_info = get_outputs.get_data_dict(
            traces_info, obs_file="waveforms_dart.txt", syn_file="synthetics_dart.txt"
        )
        values = ["dart"]
        for components in values:
            plot_waveform_fits(traces_info, components, "dart", start_margin=0)
    if "tele_body" in used_data_type:
        ...
        values = [["BHZ"], ["SH"]]
        for components in values:
            plot_waveform_fits(traces_info, components, "tele_body", event=event)
    if "surf_tele" in used_data_type:
        ...
        values = [["BHZ"], ["SH"]]
        for components in values:
            plot_waveform_fits(traces_info, components, "surf_tele", event=event)
    if "strong_motion" in used_data_type:
        ...
        for components in values:
            plot_waveform_fits(traces_info, components, "strong_motion", event=event)
    if "cgps" in used_data_type:
        ...
        for components in values:
            plot_waveform_fits(traces_info, components, "cgps", event=event)
    return
...
def _PlotSlipDistribution(segments, point_sources, solution):
    ...
    for i_segment, (segment, slip_seg, rake_seg, ps_seg) in enumerate(
        ...
        if i_segment == 0:
            ax.plot(0, 0, "w*", ms=20)
        ax, im = __several_axes(slip_seg, segment, ps_seg, ax, max_val=max_slip)
        cbar_ax = fig.add_axes([0.85, 0.15, 0.05, 0.7])
        cb = fig.colorbar(im, cax=cbar_ax)
        cb.set_label("Slip (cm)")
        plt.savefig("SlipDist_plane{}.png".format(i_segment), bbox_inches="tight")
        plt.close()
    return


def _PlotSlipDist_Compare(segments, point_sources, input_model, solution):
    ...
    for i_segment, (
        ...
        ax0, im = __several_axes(
            slip_seg, segment, ps_seg, ax0, max_val=max_slip, autosize=False
        )
        ax1, im = __several_axes(
            slip_seg2, segment, ps_seg, ax1, max_val=max_slip, autosize=False
        )
        ...
        cb.set_label("Slip (cm)")
        plt.savefig(
            "SlipDist_Compare_plane{}.png".format(i_segment), bbox_inches="tight"
        )
        plt.close()
    return
...
def _PlotMap(
    ...
    if event is not None:
        ...
        point_sources2 = [
            ps_seg for segment, ps_seg in zipped if segment["event"] == event
        ]
...
def _PlotInsar(
    tensor_info, segments, point_sources, default_dirs, insar_points, los="ascending"
):
...
def _PlotComparisonMap(tensor_info, segments, point_sources, input_model, solution):
...
def _plot_moment_rate_function(segments_data, shear, point_sources, event=None):
    ...
    plt.text(
        0.5 * max(time), 0.95 * max(mr), "$M_0$: {:.2E} $Nm$".format(seismic_moment)
    )
    plt.text(0.5 * max(time), 0.85 * max(mr), "$M_w$: {:.2f}".format(magnitude))
    plt.grid("on")
...
def __several_axes(data, segment, point_source_seg, ax, max_val=None, autosize=True):
    """ """
    (
        stk_subfaults,
        dip_subfaults,
...
if __name__ == "__main__":
    ...
    if args.strong or args.gps:
        ...
        _PlotComparisonMap(tensor_info, segments, point_sources, input_model, solution)
    if args.insar:
        solution = get_outputs.read_solution_static_format(segments)
