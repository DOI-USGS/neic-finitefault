...
def read_solution_fsp_format(file_name, custom=False):
    ...
    if n_segments == 1:
        ...
        for i_segment in range(n_segments):
            ...
            subfaults_seg = (
                int(jk[line0 + 6][3]) if not custom else int(jk[line0 + 7][3])
            )
...
def get_data_dict(
    traces_info, syn_file=None, observed=True, checker=False, obs_file=None, margin=10
):
...
def _get_synthetic_from_chen(file, syn_file):
    ...
    lines0 = [i for i, line in enumerate(lines) if len(line) > 2]
    index = next(i for i in lines0 if lines[i][2] == name and lines[i][3] in channel)
    npts = int(lines[index][0])
    synthetic = [float(real) for real, imag in lines[index + 1 : index + npts]]
