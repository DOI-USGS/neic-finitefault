...
def gf_retrieve(used_data_type, default_dirs):
    ...
    if "tele_body" in used_data_type:
        ...
        logger1 = ml.create_log("body_wave_GF", os.path.join("logs", "green_tele_log"))
    ...
    if "cgps" in used_data_type:
        ...
        logger3 = ml.create_log("get_cgps_GF", os.path.join("logs", "green_cgps_log"))
        logger3.addHandler(ch)
        p3 = subprocess.Popen(
            [green_fun_str, "cgps"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
    ...
    if "gps" in used_data_type:
        ...
        logger4 = ml.create_log("GPS_GF", os.path.join("logs", "green_gps_log"))
        logger4.addHandler(ch)
        p4 = subprocess.Popen(
            [green_fun_gps, "gps"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
    ...
    if "insar" in used_data_type:
        ...
        logger5 = ml.create_log("INSAR_GF", os.path.join("logs", "green_insar_log"))
        logger5.addHandler(ch)
        p5 = subprocess.Popen(
            [green_fun_gps, "insar"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def fk_green_fun0(dt, tensor_info, default_dirs, gf_bank=None):
    ...
    if dt < 0.9:
        with open(
            os.path.join(default_dirs["strong_motion_gf_bank"], "gf_banks_data.json"),
            "r",
        ) as f:
            dicts = json.load(f)
    else:
        with open(
            os.path.join(default_dirs["cgps_gf_bank"], "gf_banks_data.json"), "r"
        ) as f:
            dicts = json.load(f)
