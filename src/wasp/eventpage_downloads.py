...
def write_Okada_displacements(directory: Union[pathlib.Path, str] = pathlib.Path()):
    ...
    # Remember pyproj assumes meters, so multiply km by 1000
    ...
    # Make Lon range from 0 to 360 (not -180 to 180) to avoid plotting issues
    ...
    with open(directory / "Solucion.txt") as sol:
        for line in sol:
            ...
            if len(array(line.split())) < 4:  # FAULT BOUNDARY LINES
                ...
                # Make rake range from -180 to 180 (not 0-360)
                if float(rake) > 180:
                    rake = float(rake) - 360
                # Make Lon range from 0 to 360 (not -180 to 180)
                if float(lon) < 0:
                    lon = float(lon) + 360
    ...
    for ksub in range(Nfaults):
        ...
        rake = RAKE[ksub]
        depth = DEP[ksub]
        ss_slip = (slip / 100.0) * cos(
            deg2rad(rake)
        )  # Slip in the strike direction (m)
        ds_slip = (slip / 100.0) * sin(deg2rad(rake))  # Slip in the dip direction (m)
        ...
        ### Turn observation points into three-column variable with z-coordinate = 0 (i.e., surface) ###
        obs_pts = array([x, y, 0 * y]).reshape(3, -1).T.copy()

        ### Find corners of subfault in question, starting with N-striking, vertically-dipping fault with the same dimensions as the model subfault ###
        fault_corners = array(
            [
        ...
        ### Then we rotate that N-striking, vertically-dipping subfault into the true strike and dip ###
        ...
        theta_dip = -deg2rad(
            90 - dip
        )  # Rotate by the complement of the dip, since dip is measured from horizontal plane
        R_dip = zeros((3, 3))
        ...
        ### Split rectangular subfault corners into 2 triangles for Okada disp calculation ###
        triangle1 = tile(
            fault_corners[0:3, :], (len(obs_pts), 1, 1)
        )  # corners of first triangle are corners 0,1,2
        corners2 = delete(
            fault_corners, 1, axis=0
        )  # corners of second triangle are corners 0,2,3
